...
function HUDButton({ onClick, children, shortcut }: { onClick: () => void; children: React.ReactNode; shortcut?: string }) {
  ...
  // Use title for accessibility hint if children are text, otherwise just the shortcut
  const title = typeof children === 'string' ? `${children} (${shortcut})` : shortcut;
...
export default function App() {
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      // Ignore repeated keys (holding down)
      if (e.repeat) return;
