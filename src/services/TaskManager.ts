...
export class TaskManager {
  ...
  getCurrentObjective(): SurgicalObjective | null {
    return this.patientCase.objectives.find(
      obj => obj.phase === this.progress.currentPhase &&
  ...
  getCurrentPhaseObjectives(): SurgicalObjective[] {
    return this.patientCase.objectives.filter(
      obj => obj.phase === this.progress.currentPhase
    );
  }
  ...
  getNextObjective(): SurgicalObjective | null {
    ...
    // Check next phase
    return this.patientCase.objectives.find(
      obj => obj.phase === this.progress.currentPhase + 1
    ) || null;
  }
  ...
  completeObjective(objectiveId: string): void {
    ...
    const timeBonus = objective.timeLimit && elapsed < objective.timeLimit
      ? 5
      : 0;
    ...
    const allPhaseComplete = phaseObjectives.every(
      obj => this.progress.completedObjectives.includes(obj.id)
    );
  ...
  private advancePhase(): void {
    ...
    const hasNextPhase = this.patientCase.objectives.some(
      obj => obj.phase === nextPhase
    );
  ...
  getPhaseProgress(): number {
    ...
    const completedInPhase = phaseObjs.filter(
      obj => this.progress.completedObjectives.includes(obj.id)
    ).length;

    return phaseObjs.length > 0
      ? (completedInPhase / phaseObjs.length) * 100
      : 0;
  }
