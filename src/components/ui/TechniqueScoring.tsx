...
export function TechniqueScoring({
  ...
  if (compact) {
    return (
      ...
      >
        <div style={{ fontSize: '2rem', fontWeight: 700, color: gradeColor }}>
          {grade}
        </div>
      ...
      <div style={{ marginBottom: '12px', borderBottom: '1px solid rgba(247, 229, 218, 0.2)', paddingBottom: '8px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'baseline', gap: '8px' }}>
            <span style={{ fontSize: '2rem', fontWeight: 700, color: gradeColor }}>
              {grade}
            </span>
...
function ScoreBar({ label, score, weight }: { label: string; score: number; weight: number }) {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '0.85rem' }}>
      ...
      <span style={{ width: '45px', textAlign: 'right', fontWeight: 600, color }}>
        {score}
      </span>
      <span style={{ width: '30px', opacity: 0.5, fontSize: '0.7rem' }}>
        {weight}%
      </span>
    </div>
  );
