...
interface CaseCardProps {
  ...
  getDifficultyColor: (difficulty: CaseDifficulty) => string;
}

function CaseCard({ patientCase, isAvailable, isCompleted, isSelected, onSelect, getDifficultyColor }: CaseCardProps) {
  ...
  const currentStyle = {
    ...
    ...((isAvailable && !isSelected && (isHovered || isFocused)) ? {
  ...
  return (
    <button
      ...
      disabled={isLocked}
      aria-label={isLocked ? `Locked case: ${patientCase.name}` : `Select case: ${patientCase.name}`}
      style={currentStyle}
