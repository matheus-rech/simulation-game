...
const styles = {
  container: {
    ...
    overflowY: 'auto' as const, // Ensure scrolling if content is tall
  },
  wrapper: {
    maxWidth: '1400px',
    width: '100%',
  ...
  patientInfo: {
    ...
    color: '#bfdbfe',
    marginBottom: '16px',
    margin: '0 0 16px 0', // Reset margins
  },
  detailsBox: {
    background: 'rgba(255, 255, 255, 0.1)',
    backdropFilter: 'blur(12px)',
...
function CaseCard({ patientCase, isAvailable, isCompleted, isSelected, onSelect }: CaseCardProps) {
  ...
  const cardStyle = {
    ...
    ...(isAvailable ? {} : styles.caseCardLocked),
    transform: (isHovered || isFocused) && isAvailable && !isSelected ? 'scale(1.05)' : 'scale(1)',
    borderColor: (isHovered || isFocused) && isAvailable && !isSelected ? '#93c5fd' : (isSelected ? '#60a5fa' : 'rgba(255, 255, 255, 0.2)'),
    // Focus ring
  ...
  return (
    <button
      type="button"
      onClick={() => isAvailable && onSelect(patientCase)}
    ...
    >
      ...
      <div style={{ fontSize: '0.875rem', color: '#d1d5db' }}>
        <div style={{ marginBottom: '8px' }}>
          ...
          <p style={{ marginTop: '4px', color: '#9ca3af', margin: '4px 0 0 0' }}>{patientCase.chiefComplaint}</p>
        </div>
