...
export function SafetyCorridorManager({
  ...
  const _getRiskColor = (level: RiskLevel): number => {
    switch (level) {
      case RiskLevel.SAFE: return 0x00ff00;      // Green
      case RiskLevel.WARNING: return 0xffff00;   // Yellow
      case RiskLevel.DANGER: return 0xff8800;    // Orange
      case RiskLevel.CRITICAL: return 0xff0000;  // Red
    }
  };
  ...
  const debugSpheres = useMemo(() => {
    ...
    Object.entries(structures).forEach(([key, position]) => {
      ...
      const structureType = key.includes('ica') ? 'ICA' :
                           key.includes('mwcs') ? 'MWCS' : 'DURA';
